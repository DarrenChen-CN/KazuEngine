// ============================================================================
// KazuEngine - Precompute Layer: Equirectangular to Cubemap Pass (Implementation)
// ============================================================================
...
EquirectToCubePass::EquirectToCubePass(Texture* equirectTexture, uint32_t size)
    : m_equirectTexture(equirectTexture)
    , m_size(size) {
}

EquirectToCubePass::~EquirectToCubePass() {
    if (!m_rhi) return;
...
PrecomputePass::OutputDesc EquirectToCubePass::outputDesc() const {
    return {
        "EnvironmentCube",
        m_size, m_size,
        1, 6,
...
void EquirectToCubePass::setOutputResource(RenderGraph::ResourceHandle handle, Image* image) {
    m_outputHandle = handle;
    m_outputImage = image;
}

void EquirectToCubePass::createPipeline() {
    VkDevice device = m_rhi->ctx().device();

    VkShaderModule shaderModule = m_rhi->shaderLib().load(
        kazu::Path::resolveShader("equirect_to_cube.comp.spv"));
...
void EquirectToCubePass::createDescriptorSet() {
    VkDevice device = m_rhi->ctx().device();
    ...
    // Storage image needs a 2D array view of the cube image.
    ...
    equirectInfo.imageView = m_equirectTexture->view();
    equirectInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    equirectInfo.sampler = m_equirectTexture->sampler();
...
void EquirectToCubePass::declare(RHI* rhi, RenderGraph* rg) {
    m_rhi = rhi;
    m_renderGraph = rg;

    EquirectToCubePass* self = this;
    rg->addPass("EquirectToCubeCompute", [&](RenderGraph::PassBuilder& b) {
    ...
    rg->addPass("EquirectToCubeFinalize", [&](RenderGraph::PassBuilder& b) {
        b.read(self->m_outputHandle);
        b.execute = [](const PassExecuteContext&) {};
    });
}

void EquirectToCubePass::create(const PassCreateContext& ctx) {
    (void)ctx;
    createPipeline();
    createDescriptorSet();
}

void EquirectToCubePass::execute(const PassExecuteContext& ctx) {
