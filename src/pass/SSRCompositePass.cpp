// ============================================================================
// KazuEngine - Pass Layer: SSR Composite Pass (Implementation)
// ============================================================================
...
SSRCompositePass::SSRCompositePass() = default;
...
void SSRCompositePass::declare(RHI* rhi, RenderGraph* rg) {
    ...
    SSRCompositePass* self = this;
    m_passHandle = rg->addPass("SSRComposite", [&](RenderGraph::PassBuilder& b) {
        b.type = RenderGraph::PassType::Compute;
        b.read(self->m_blurredTempHandle);
...
void SSRCompositePass::create(const PassCreateContext& ctx) {
    m_rhi = ctx.rhi;
...
void SSRCompositePass::createPipeline() {
    VkDevice device = m_rhi->ctx().device();
    ...
    VkShaderModule cs = m_rhi->shaderLib().load(
        kazu::Path::resolveShader("ssr_blur_v.comp.spv"));
...
void SSRCompositePass::createDescriptorSet() {
    VkDevice device = m_rhi->ctx().device();
    ...
    blurredInfo.sampler = m_sampler;
    blurredInfo.imageView = blurredTempView;
    blurredInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

    VkDescriptorImageInfo sceneColorInfo{};
    sceneColorInfo.sampler = m_sampler;
    sceneColorInfo.imageView = sceneColorView;
    sceneColorInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

    std::array<VkDescriptorImageInfo, 3> infos = { outInfo, blurredInfo, sceneColorInfo };
...
void SSRCompositePass::execute(const PassExecuteContext& ctx) {
