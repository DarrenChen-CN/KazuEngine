// ============================================================================
// KazuEngine - Pass Layer: SSAO Blur Pass (Implementation)
// ============================================================================
...
void SSAOBlurPass::setInputAO(RenderGraph::ResourceHandle ao) {
    m_inputAOHandle = ao;
}

void SSAOBlurPass::declare(RHI* rhi, RenderGraph* rg) {
    m_blurredAOHandle = rg->addTexture("SSAOBlurred",
        {.width  = rhi->extent().width,
         .height = rhi->extent().height,
         .format = VK_FORMAT_R8_UNORM,
         .usage  = VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT});

    SSAOBlurPass* self = this;
    m_passHandle = rg->addPass("SSAOBlur", [&](RenderGraph::PassBuilder& b) {
        b.type = RenderGraph::PassType::Compute;
        b.read(self->m_inputAOHandle);
        b.writeStorageImage(self->m_blurredAOHandle);
...
void SSAOBlurPass::create(const PassCreateContext& ctx) {
    m_rhi = ctx.rhi;
...
void SSAOBlurPass::createPipeline() {
    VkDevice device = m_rhi->ctx().device();
    ...
    VkShaderModule cs = m_rhi->shaderLib().load(
        kazu::Path::resolveShader("ssao_blur.comp.spv"));
...
void SSAOBlurPass::createDescriptorSet() {
    VkDevice device = m_rhi->ctx().device();
    ...
    VkImageView blurredView = m_renderGraph->getImageView(m_blurredAOHandle);
    VkImageView inputView   = m_renderGraph->getImageView(m_inputAOHandle);

    VkDescriptorImageInfo outInfo{};
    outInfo.imageView = blurredView;
...
void SSAOBlurPass::execute(const PassExecuteContext& ctx) {
